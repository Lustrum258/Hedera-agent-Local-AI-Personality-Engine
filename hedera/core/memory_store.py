...
class MemoryStore:
    ...
    def get_session_messages(self, session_id: str, limit: int = 500) -> tuple:
        ...
        try:
            rows = conn.execute(
                ...
                "OR (role = 'system' AND content = '[PROCESSING]')) "
    ...
    def delete_session(self, session_id: str):
        ...
        try:
            ...
            conn.execute("DELETE FROM slider_snapshots WHERE session_id = ?", (session_id,))
            conn.commit()
        finally:
            conn.close()
    ...
    def clear_all_sessions(self) -> int:
        ...
        try:
            ...
            rows = conn.execute(
                "SELECT session_id FROM sessions WHERE session_id NOT IN ('_api', '_reflection', '_default', '_experience', '_admin', '_cross_session_prompt')"
            ).fetchall()
            count = len(rows)
            for (sid,) in rows:
